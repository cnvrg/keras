...
class Sequential(Model):
    ...
    def from_config(cls, config, custom_objects=None):
        #Nir R  05/11/2018 Trying to SOLVE multi_gpu bug in model_from_json
        if isinstance(config, dict):
            if 'class_name' not in config['layers'][0] or config['layers'][0]['class_name'] == 'Merge':
            ...
            for conf in config['layers']:
                ...
                model.add(layer)
            return model
        elif 'class_name' not in config[0] or config[0]['class_name'] == 'Merge':
            return cls.legacy_from_config(config)
